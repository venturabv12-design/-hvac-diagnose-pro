...
// Plans we must NOT overwrite (real paying customers / admins).
...
(async () => {
  ...
  const targets = users.filter(u => {
    if (KEEP.includes(u.plan)) return false;              // never overwrite paying/admin/already-founder
    const created = u.created_at ? new Date(u.created_at).getTime() : 0;
    return created < CUTOFF_MS;                            // created before the wall = founding tech
  });

  console.log(`Total accounts: ${users.length}`);
  console.log(`Already protected (paying/admin/founder): ${users.filter(u => KEEP.includes(u.plan)).length}`);
