...
async function findGaps() {
  ...
  // Skip anything already covered — a tech may have asked before a manual landed.
  const out = [];
  for (const g of gaps) {
    if (out.length >= MAX_PER_NIGHT) break;
    const have = await sb('GET',
      `manual_chunks?select=id&brand=eq.${encodeURIComponent(g.brand)}` +
      `&doc_id=like.*${encodeURIComponent(String(g.model).toLowerCase())}*&limit=1`).catch(() => null);
