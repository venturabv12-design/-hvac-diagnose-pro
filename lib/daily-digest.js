...
// ── ONE SEND PER DAY, ACROSS RESTARTS ──────────────────────────────────────────
// 2026-08-30 — Brandon got FIVE identical digests between 7:03 and 7:08. The only
// guard was `lastDay`, a variable inside the process, so every restart during the
// send hour started a fresh process that believed it had never sent. Railway
// restarts this app routinely (deploys, platform moves, health checks), so the
// digest fired once per restart.
//
// index.js already solved this exact class of bug for traffic counters — "a redeploy
// wipes rowId but NOT the day's row" — but the pattern was never applied here. Same
// lesson as the trading leverage bug: fix it everywhere the pattern exists, not only
// where it happened to surface.
//
// Claim BEFORE sending, then read back who claimed first. Insert-then-verify rather
// than check-then-insert, because two processes booting together would both pass a
// plain existence check. Whoever owns the earliest row sends; everyone else stands
// down. Survives restarts AND multiple replicas.
async function claimSendSlot(day) {
  const url = process.env.SUPABASE_URL, key = process.env.SUPABASE_SERVICE_KEY;
  if (!url || !key) return true;   // no DB configured (dev) — don't block the send
  const h = { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' };
  try {
    const admins = await fetch(`${url}/rest/v1/users?select=id&role=eq.admin&limit=1`,
      { headers: h, signal: AbortSignal.timeout(10000) }).then(r => r.json());
    const uid = admins && admins[0] && admins[0].id;
    if (!uid) return true;

    const mine = await fetch(`${url}/rest/v1/events`, {
      method: 'POST', headers: { ...h, Prefer: 'return=representation' },
      signal: AbortSignal.timeout(10000),
      body: JSON.stringify({ user_id: uid, type: 'digest_sent', payload: { date: day } }),
    }).then(r => r.json());
    const myId = mine && mine[0] && mine[0].id;
    if (!myId) return true;        // couldn't claim — fail toward sending, not toward silence

    const first = await fetch(
      `${url}/rest/v1/events?type=eq.digest_sent&payload->>date=eq.${day}` +
      `&select=id&order=created_at.asc,id.asc&limit=1`,
      { headers: h, signal: AbortSignal.timeout(10000) }).then(r => r.json());
    const winner = first && first[0] && first[0].id;
    if (winner && String(winner) !== String(myId)) {
      log(`already sent for ${day} — standing down`);
      return false;
    }
    return true;
  } catch (e) {
    // A digest that never arrives is worse than one that arrives twice, so a DB
    // failure lets the send through rather than silently swallowing the day.
    log('claim check failed, sending anyway:', e.message);
    return true;
  }
}
...
async function sendOnce(opts) {
  ...
  try {
    ...
    // Only the SCHEDULED send claims the day. A manual hit on /api/admin/digest is a
    // deliberate "send me this now" and must never be swallowed by the day's claim.
    if ((opts && opts.scheduled) && !(await claimSendSlot(new Date().toISOString().slice(0, 10)))) {
      _last = { at: new Date().toISOString(), skipped: 'already sent today', subject };
      return _last;
    }
...
// WEEKLY BY DEFAULT, NOT DAILY.
// Brandon, 2026-09-13: "I got so many emails that you guys are sending me daily... it's
// just gonna fill up my work email and even my personal. I don't want that. I want
// weekly emails." A digest nobody opens is worse than no digest — it trains him to
// swipe away mail from this domain, which is the same mail an outage arrives on.
// DIGEST_DAY: 0=Sunday..6=Saturday. Set DIGEST_DAY=* to restore the daily cadence.
const SEND_DAY = process.env.DIGEST_DAY === undefined ? '0' : String(process.env.DIGEST_DAY);
function start() {
  ...
  const tick = () => {
    ...
    if (SEND_DAY !== '*' && now.getDay() !== Number(SEND_DAY)) return;
    if (now.getHours() === SEND_HOUR && lastDay !== day) {
      lastDay = day;
      sendOnce({ scheduled: true }).catch(() => {});
    }
  };
  const t = setInterval(tick, 10 * 60 * 1000);
  if (t.unref) t.unref();
  const DAYS = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
  log(`scheduled — ${SEND_HOUR}:00 local, ${SEND_DAY === '*' ? 'daily' : DAYS[Number(SEND_DAY)] + 's only'}`);
}
