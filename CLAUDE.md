...
- **Supabase** (REST, no SDK) — accessed via the `supabase(method, table, body, query)` helper at the top of the file using the service-role key. **There is currently a divergence**: `/api/customers`, `/api/jobs`, `/api/reminders`, `/api/refrigerant-log`, `/api/knowledge` call `supabase.from(...).select()...` as if it were a Supabase JS client — that pattern won't work against the helper as written and those routes will throw. If you touch any of those routes, rewrite them to use the `supabase()` REST helper (see `/api/auth/*` for the pattern) or introduce a real Supabase client.
