...
const SYSTEM = [
  ...
  '',
  'EQUIPMENT IDENTITY (so Mike can file this in the shared library for the next tech on the same unit):',
  'A. Read the equipment identity printed ON the diagram sheet and fill the equipment{} object: model_id (the model/spec number), series/family, brand, and the diagram/label part-number — whatever is actually printed. This is how the redraw gets keyed and reused.',
  'B. Best-effort ONLY. If the model number is not printed on the sheet or is not clearly legible, leave that field null — do NOT guess or infer it. A wrong model files the wrong diagram for the next tech (fail toward distrust).',
  '',
